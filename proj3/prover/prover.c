...
//Created in class
int unifyPred(int sent1,int p1,int sent2,int p2)
{
    int param;
    int numAssign = 0;
    ...
    param1 = sentlist[sent2].param[p2];

    //now we know the predicates are the same
    //after the two if checks above
    for(param=0;param< predlist[sentlist[sent1].pred[p1]].numparam;param++)
    {
	//need to walk assignment list and make them
	inti i;
	for(i = 0; i<numAssing;i++)
	{
	    if(!memcmp(&(param1[param],theta[i].var, sizeOf(Parameter))) param1[param] = *(theta[i].val);
	    if(!memcmp(&(param1[param],theta[i].var, sizeOf(Parameter))) param2[param] = *(theta[i].val);
	}
	//problem we are running into right off tehe bat
	//we passed in indices to these two sentences, and the predicates we are
	//dealing with. 
	//we do not want to change the sentences, once they hit the databade
	//we need to never change them again
	//so make a copy above the for look
	//array of parameters
...
//I beleive he made this in class, I didn't see another one in here
int tryResolution(int sent1, int sent2)
{
	...
	if(numAssign>=0)
	{
	   //1st thing to do: use sentptr
	   //use sentlist[sentptr]
	   //then at very end do 
	   //sentptr++
	   int neg[MAXPRED];
	   int pred[MAXPRED];
	   char param[
	   //fill up with everything from these two sentences except the two
	   //matching predicates
	   //everything but p1 and p2
	   //take pred and put in pred list
	   //then do variable replacements
	   //then standardize apart
	   //see a constant and skip it
	   //see a variable 
	   //go to variable counter and place it with a new variable

	   
	   //walk the assignment list and perform assignments on all the
	   //predicate paramters
	   //in char
