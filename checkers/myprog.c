...
void FindBestMove(int player)
{
	...
	//create a for loop, for all the legal moves and pick the best move
	//also with a number to keep track
...
double evalBoard(State *currBoard)
{
	...
	for(int i=0;i<8;i++){
	    for(int y=0;y<8;y++){
			if(king(board[x][y])){
				if(color[x][y] == 1) red_total+=2;
				else white_total+=2;
			else if(piece(board[x][y])){ //this must be a pawn since we've already checked for king
...
double maxVal(char currBoard[8][8], double alpha, double beta, int depth)
{
	...
	for(int i=0;i<state.numLegalMoves;i++)
	{
		...
		//check alpha and return alpha or beta depending
	}

	return alpha;
}

void performMove(char currBoard[8][8], double alpha, double beta, int depth){
